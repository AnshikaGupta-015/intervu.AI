    ...
    <div className='bg-[#f3f3f3] flex justify-center px-4 pt-6'>
      ...
      className='w-full max-w-6xl bg-white rounded-[24px] shadow-sm border border-gray-200 px-8 py-4 flex justify-between items-center relative'>
        <div className='flex items-center gap-3 cursor-pointer'>
          ...
          <h1 className='font-semibold hidden md:block text-lg'>Intervu.AI</h1>
             
        </div>
          <div className='flex items-center gap-6 relative'>
              <div className='relative'>
                <button 
                onClick={()=>{setShowCreditPopup
                (!showCreditPopup);
                setShowUserPopup(false)
                }}
              ...
              <div className='relative'>
                ...
                {showUserPopup && (
                  <div className='absolute right-0 mt-3 w-48 bg-white shaadow-xl border borger-gray-200 rounded-xl p-4 z-50'>
                    <p className='text-md text-blue-500 font-medium mb-1'>{userData?.name}</p>

                    <button 
                    onClick={()=>navigate("/history")}
                    className='w-full text-left text-sm py-2 hower:text-black text-gray-600'>InterView History</button>
                    <button 
